...
const { botToken, adminId, adminUsername, supportUsername, website, addresses } = require("./config");
const { plans } = require("./plans");
const { addPending, updatePending } = require("./storage");

const bot = new Telegraf(botToken);
const userState = new Map();
const infoChannelUrl = "https://t.me/polybotinfo";
...
const userCommands = [
  ...
  { command: "software", description: "Software and web panel" },
  { command: "support", description: "Contact support" },
  { command: "channel", description: "Polybot info channel" },
...
function mainMenu() {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("🖥️ Software / Web", "software"), Markup.button.callback("🎧 Support", "support")],
    [Markup.button.url("📢 Polybot Info Channel", infoChannelUrl)]
  ]);
}
...
function buyMenu() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("◈ Core — $69", "plan:core")],
    [Markup.button.callback("✦ Prime — $99", "plan:prime")],
    [Markup.button.callback("♛ Elite — $199", "plan:elite")],
    [Markup.button.callback("📈 Compare Plans", "plans_info")],
    [Markup.button.url("📢 Results / Updates Channel", infoChannelUrl)],
...
function plansInfoMenu() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("◈ Core Details", "plan_info:core")],
    [Markup.button.callback("✦ Prime Details", "plan_info:prime")],
    [Markup.button.callback("♛ Elite Details", "plan_info:elite")],
    [Markup.button.callback("💳 Buy Access", "buy")],
    [Markup.button.url("📢 Results / Updates Channel", infoChannelUrl)],
    [Markup.button.callback("← Back", "menu")]
  ]);
...
function startText() {
  return `<b>Polybot</b> — AI-powered trading system for Polymarket 5-minute markets.

Polybot is built for traders who want faster execution, structured decisions, and AI-assisted market analysis in short 5-minute windows.

<b>How you can use it:</b>
• Install the desktop software on Windows PC
• Install the desktop software on Mac
• Or use the online control panel at <b>${website}</b>

Users can choose how their system works: local desktop software on PC / Mac or the web panel. All plans include full market access. Higher plans add more OpenClaw compute power for faster scanning and stronger processing.

<b>What Polybot helps with:</b>
...
Choose your Polybot compute level.

All plans include full Polybot access:
• Web panel at <b>${website}</b>
• PC / Mac software
• Paper Mode
• Live Mode
• BTC / ETH / SOL 5-minute markets
...
The difference is compute power.

Higher plans connect more OpenClaw workers to your Polybot system, giving faster scanning, stronger processing, and better execution speed during short 5-minute market windows.`;
}
...
The difference between plans is compute power: more OpenClaw workers, faster scanning, stronger processing, and better execution speed.
...
• Fastest scanning and processing
...
Polybot combines AI-assisted market analysis, execution workflow, and user-controlled access.

Users can choose how they want to run their system:
• PC software
• Mac software
• Web panel at <b>${website}</b>
...
7. Choose how you want to use Polybot:
   • Install the PC version
   • Install the Mac version
   • Use the web panel at <b>${website}</b>
8. Start in Paper Mode
9. Switch to Live Mode when ready

Support is available if you need help with payment, setup, activation, or choosing the best setup for your device.`;
}

function softwareText() {
  return `<b>Software & Web Panel</b>

Polybot gives users two ways to work with the system.

<b>Option 1 — Desktop software</b>
Install Polybot directly on:
• Windows PC
• Mac

This is the best option for users who want a local trading setup and direct control from their own machine.

<b>Option 2 — Web panel</b>
Use the online panel:

<b>${website}</b>

The web panel is made for users who want easier access, account control, setup flow, and bot management from the browser.

<b>Included in the system:</b>
• Market scanner
• Paper Mode
• Live Mode
...
Info channel: https://t.me/polybotinfo
...
The AI layer helps the system move beyond simple button automation. It supports market condition analysis, strategy checks, signal filtering, execution flow, and risk-aware decision structure.
...
<b>User setup options:</b>
• Run Polybot as PC software
• Run Polybot as Mac software
• Use the web panel at <b>${website}</b>
...
function planInfoText(plan) {
  const extra = {
    core: "Core includes the full Polybot system with 1 OpenClaw worker. Good for users who want full access with standard compute power.",
    prime: "Prime includes the full Polybot system with 3 OpenClaw workers. Better for users who want faster scanning, stronger processing, and priority support.",
    elite: "Elite includes the full Polybot system with all available OpenClaw workers and maximum compute power. Built for users who want the strongest setup."
  }[plan.id];
...
<b>Compute Power:</b> ${plan.compute}
<b>Access:</b> ${plan.accessLevel}
<b>Markets:</b> ${plan.markets}
<b>Processing:</b> ${plan.processing}
<b>Support:</b> ${plan.support}
...
bot.command("hidekeyboard", async (ctx) => {
  return ctx.reply("Keyboard hidden.", Markup.removeKeyboard());
});
...
bot.command("channel", (ctx) => {
  return ctx.reply(`<b>Polybot Info Channel</b>

Results, updates, user PnL posts, setup notes, and important announcements:

${infoChannelUrl}`, {
    parse_mode: "HTML",
    disable_web_page_preview: true,
    ...Markup.inlineKeyboard([
      [Markup.button.url("📢 Open Polybot Info", infoChannelUrl)],
      [Markup.button.callback("← Back", "menu")]
    ])
  });
});
...
bot.command("support", (ctx) => answerAndSend(ctx, assets.support, supportText(), Markup.inlineKeyboard([
  ...
  [Markup.button.url("📢 Polybot Info", infoChannelUrl)],
...
You can also use the web panel:
<b>${website}</b>
...
bot.hears("🖥️ Software / Web", (ctx) => answerAndSend(ctx, assets.software, softwareText(), Markup.inlineKeyboard([
  [Markup.button.callback("🪟 Download for PC", "download:pc")],
  [Markup.button.callback(" Download for Mac", "download:mac")],
  [Markup.button.callback("← Back", "menu")]
])));
...
bot.launch().then(async () => {
  ...
  console.log("Build: v28 lower subscription prices");
