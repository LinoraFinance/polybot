...
## v20 remove menu updated message

- Removed the visible `Menu updated.` message from `/start` and `/menu`.
- `/start` now opens normally without extra service text.
- `/hidekeyboard` is still available if the old bottom keyboard needs to be hidden manually.


## v21 hard cleanup

- Hard removed any `Menu updated.` / `Menu enabled.` service messages from `/start` and `/menu`.
- Added Railway log marker: `Build: v21 no service start message`.


## v22 software + web panel copy

- Updated bot copy to explain that users can choose how their system works.
- Added setup options:
  - Windows PC software
  - Mac software
  - Web panel at `polytradebot.xyz`
- Removed previous positioning that implied there is no website.
- Updated Software Info to `Software & Web Panel`.


## v23 three functionality plans

Plans changed from day-based access to functionality-based access:
...
- Edge
- Lifetime
- 1 day / 7 days / 30 days plan positioning

New plan logic:

- Core: entry/testing access
- Prime: main plan with Live Mode, BTC / ETH / SOL markets, advanced AI execution, faster scanning, priority support
- Elite: full system access, strongest AI workflow, fastest priority, private setup, future modules


## v24 compute-based plans

Plans now differ only by compute power, not by market access.
...
Plan differences:
- Core — $199 — 1 OpenClaw worker
- Prime — $1,499 — 3 OpenClaw workers
- Elite — $3,999 — all OpenClaw workers / maximum compute power


## v25 syntax fix

Fixed a JavaScript syntax error caused by a broken `.join("\n")` string in `src/bot.js`.

Railway log marker:
`Build: v25 syntax fix`


## v26 info channel

Added Telegram info channel:
https://t.me/polybotinfo

Changes:
- Added channel button to main menu
- Added channel button to buy/plans/support flows
- Added `/channel` command
- Updated command list
- Railway log marker: `Build: v26 info channel added`


## v27 new subscription prices
...
Railway log marker:
`Build: v27 new subscription prices`


## v28 lower subscription prices
...
Railway log marker:
`Build: v28 lower subscription prices`
